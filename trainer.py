...
class CLTrainer(Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        if self.optimizer is None:
            ...
            lambda_lr = self.args.lambda_learning_rate

            # 获取模型的所有参数
            model_parameters = list(self.model.parameters())
            lambda_parameters = model_parameters.pop()

            # 创建优化器，设置不同的学习率
            optimizer_grouped_parameters = [
                {'params': model_parameters, 'lr': base_lr},
                {'params': [lambda_parameters], 'lr': lambda_lr},  # lambda的学习率
            ]

            self.optimizer = AdamW(optimizer_grouped_parameters, lr=base_lr)
    ...
    def compute_loss(self, model, inputs):
        ...
        if self.args.adv_training:
            ...
            self.fgm.attack(epsilon=self.args.adv_eps)
            model.zero_grad()
            outputs = model(**inputs)
        # with open(r"E:\Project\EmdForBlock\SimCSE\result\loss.txt", "a") as f:
        #     f.write(str(outputs["loss"].item()) + "\n")
